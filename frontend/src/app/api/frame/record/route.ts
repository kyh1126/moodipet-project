...
export async function POST(req: NextRequest) {
  try {
    ...
    const frameResponse = {
      frames: [
        {
          image: `${process.env.NEXT_PUBLIC_BASE_URL}/api/og?emotion=${encodeURIComponent(emotion)}&ment=${encodeURIComponent(randomMent)}&status=recorded`,
          buttons: [
            {
              label: "다시 기록하기",
              action: "post",
            ...
            },
            {
              label: "웹사이트 방문",
              action: "link",
              target: `${process.env.NEXT_PUBLIC_BASE_URL}/moodipet`
            }
          ]
        }
      ]
