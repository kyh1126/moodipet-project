...
테스트 자동화(scripts/recordMoodAuto.js) 사용을 위해 hardhat.config.ts를 hardhat.config.ts.bak로 변경한 경우,
실제 배포/운영 시에는 반드시 hardhat.config.ts.bak를 hardhat.config.ts로 복원해야 합니다.
